...
nock('https://github.com')
  .get('/electron/fiddle/releases/download/v0.35.1/x64.RELEASES')
  .reply(404)
  .get('/electron/fiddle/releases/download/v0.35.1/ia32.RELEASES')
  .reply(404)
  .get('/electron/fiddle/releases/download/v0.35.1/arm64.RELEASES')
  .reply(404)
  .get('/electron/fiddle/releases/download/v0.36.0/x64.RELEASES')
  .reply(404)
  .get('/electron/fiddle/releases/download/v0.36.0/ia32.RELEASES')
  .reply(404)
  .get('/electron/fiddle/releases/download/v0.36.0/arm64.RELEASES')
  .reply(404)
  .get('/owner/repo/releases/download/1.0.0/x64.RELEASES')
  .reply(404)
  .get('/owner/repo/releases/download/1.0.0/ia32.RELEASES')
  .reply(404)
  .get('/owner/repo/releases/download/1.0.0/arm64.RELEASES')
  .reply(404)
  .get('/owner/repo-win32-zip/releases/download/v1.0.0/x64.RELEASES')
  .reply(404)
  .get('/owner/repo-win32-zip/releases/download/v1.0.0/ia32.RELEASES')
  .reply(404)
  .get('/owner/repo-win32-zip/releases/download/v1.0.0/arm64.RELEASES')
  .reply(404)
  .get('/owner/repo-no-releases/releases/download/v1.0.0/x64.RELEASES')
  .reply(404)
  .get('/owner/repo-no-releases/releases/download/v1.0.0/ia32.RELEASES')
  .reply(404)
  .get('/owner/repo-no-releases/releases/download/v1.0.0/arm64.RELEASES')
  .reply(404)
...
describe('Updates', () => {
  ...
  describe('Routes', () => {
    ...
    it('invalid semver in release', async () => {
      const res = await fetch(`${address}/owner/repo-invalid-semver/darwin/0.0.0`);
      expect(res.status).toBe(404);
    });

    it('exists but has no releases', async () => {
      for (let i = 0; i < 2; i++) {
        const res = await fetch(`${address}/owner/repo-without-releases/darwin/0.0.0`);
        expect(res.status).toBe(404);
      }
    });
  ...
  describe('Platforms', () => {
    describe('Darwin', () => {
      it('.zip', async () => {
        for (let i = 0; i < 2; i++) {
          ...
          res = await fetch(`${address}/owner/repo-darwin/darwin-universal/0.0.0`);
          expect(res.status).toBe(200);
          body = await res.json();
      ...
      it('missing asset', async () => {
        let res = await fetch(`${address}/owner/repo-win32-zip/darwin-x64/0.0.0`);
        expect(res.status).toBe(404);
      ...
      describe('darwin universal assets', () => {
        it('use universal asset if platform-specific asset not available', async () => {
          const res = await fetch(`${address}/owner/repo-universal/darwin-x64/0.0.0`);
          expect(res.status).toBe(200);
        ...
        it('skip universal asset if platform-specific asset available', async () => {
          const res = await fetch(`${address}/owner/repo-universal/darwin-arm64/0.0.0`);
          expect(res.status).toBe(200);
          const body = (await res.json()) as any;
    ...
    describe('Win32', () => {
      ...
      it('win32-x64', async () => {
        for (let i = 0; i < 2; i++) {
          let res = await fetch(`${address}/owner/repo-win32-zip/win32-x64/0.0.0`);
      ...
      it('win32-arm64', async () => {
        for (let i = 0; i < 2; i++) {
          const res = await fetch(`${address}/owner/repo-win32-zip/win32-arm64/0.0.0`);
          expect(res.status).toBe(200);
          const body = (await res.json()) as any;
      ...
      it('win32-ia32', async () => {
        for (let i = 0; i < 2; i++) {
          const res = await fetch(`${address}/owner/repo-win32-zip/win32-ia32/0.0.0`);
          expect(res.status).toBe(200);
          const body = (await res.json()) as any;
      ...
      describe('RELEASES', () => {
        it('win32-x64', async () => {
          for (let i = 0; i < 2; i++) {
            const res = await fetch(`${address}/owner/repo/win32-x64/0.0.0/RELEASES?some-extra`);
            expect(res.status).toBe(200);
            const body = await res.text();
            expect(body).toMatch(
          ...
          for (let i = 0; i < 2; i++) {
            const res = await fetch(`${address}/owner/repo/win32/0.0.0/RELEASES?some-extra`);
            expect(res.status).toBe(200);
            const body = await res.text();
            expect(body).toMatch(
              /^[^ ]+ https:\/\/github.com\/owner\/repo\/releases\/download\/[^/]+\/name.nupkg [^ ]+$/,
        ...
        it('win32-ia32', async () => {
          for (let i = 0; i < 2; i++) {
            const res = await fetch(`${address}/owner/repo/win32-ia32/0.0.0/RELEASES?some-extra`);
            expect(res.status).toBe(200);
            const body = await res.text();
            expect(body).toMatch(
              /^[^ ]+ https:\/\/github.com\/owner\/repo\/releases\/download\/[^/]+\/name.nupkg [^ ]+$/,
        ...
        it('handles missing RELEASES files', async () => {
          for (let i = 0; i < 2; i++) {
            const res = await fetch(`${address}/owner/repo-no-releases/win32-x64/0.0.0/RELEASES`);
            expect(res.status).toBe(404);
          }

          for (let i = 0; i < 2; i++) {
            const res = await fetch(`${address}/owner/repo-no-releases/win32/0.0.0/RELEASES`);
            expect(res.status).toBe(404);
          }
        });
      });
...
describe('electron/fiddle', () => {
  ...
  describe('first updates to v0.35.1', () => {
    it('on macOS', async () => {
      for (const platform of ['darwin', 'darwin-x64', 'darwin-arm64']) {
        for (let i = 0; i < 2; i++) {
          const res = await fetch(`${address}/electron/fiddle/${platform}/0.34.0`);
          expect(res.status).toBe(200);
    ...
    it('not on Windows', async () => {
      for (const platform of ['win32', 'win32-x64', 'win32-ia32', 'win32-arm64']) {
        for (let i = 0; i < 2; i++) {
          const res = await fetch(`${address}/electron/fiddle/${platform}/0.34.0`);
          expect(res.status).toBe(200);
  ...
  describe('updates to latest', () => {
    it('on macOS', async () => {
      for (const platform of ['darwin', 'darwin-x64', 'darwin-arm64']) {
        for (let i = 0; i < 2; i++) {
          const res = await fetch(`${address}/electron/fiddle/${platform}/0.35.1`);
          expect(res.status).toBe(200);
